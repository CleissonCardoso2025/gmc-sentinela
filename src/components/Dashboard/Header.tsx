    ...
    <header className="bg-gcm-600 fixed w-full h-16 z-50 px-6 flex items-center justify-between shadow-md animate-fade-in">
      ...
      <div className="flex items-center space-x-4">
        ...
        <Button
          variant="ghost"
          size="icon"
          className="relative bg-gcm-700/70 hover:bg-gcm-700 text-white rounded-full h-10 w-10 transition-all duration-300"
        >
          <Bell className="h-5 w-5" />
          {notifications > 0 && (
            <span className="absolute top-0 right-0 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center animate-scale-in">
              {notifications}
            </span>
          )}
        </Button>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button 
              variant="ghost" 
              className="bg-gcm-700/70 hover:bg-gcm-700 text-white flex items-center space-x-2 pl-2 pr-3 transition-all duration-300"
            >
