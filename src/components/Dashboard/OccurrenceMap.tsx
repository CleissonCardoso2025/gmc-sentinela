...
const OccurrenceMap: React.FC = () => {
  ...
  return <Card className="w-full overflow-hidden shadow-md relative animate-fade-up">
      <div className="absolute top-4 left-4 z-10">
        <h2 className="text-lg font-semibold text-gray-800">Mapa de Ocorrências</h2>
      </div>
      
      {isLoading ? <div className="w-full h-[400px] p-4 flex items-center justify-center">
          <div className="space-y-4 w-full">
            <Skeleton className="h-[350px] w-full rounded-md" />
          </div>
        </div> : <div id="occurrenceMap" ref={mapRef} className="w-full h-[400px] bg-gray-50" style={{
      ...
      filter: "grayscale(30%)"
          ...
          <div className="w-full h-full relative p-4 pt-12">
            ...
            <div className="absolute w-full h-full flex items-center justify-center text-gray-400 font-light italic">
              <p>Mapa  com localizações das ocorrências</p>
            </div>
          </div>
        </div>}
    </Card>;
