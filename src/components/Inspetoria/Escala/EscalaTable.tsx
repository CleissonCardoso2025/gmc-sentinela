...
import { Edit, UserPlus, Trash2, Shield, User } from "lucide-react";
            ...
            groupedData.map((group) => (
              <React.Fragment key={group.guarnicao}>
                {group.agents.map((agent, agentIndex) => (
                  <TableRow key={agent.id} className={agentIndex === 0 ? 'border-t-2 border-t-primary/20' : ''}>
                    {agentIndex === 0 ? (
                      <TableCell rowSpan={group.agents.length} className="whitespace-nowrap bg-muted/30 align-top">
                        ...
                        <div className="text-xs text-muted-foreground mt-3">
                          ...
                          <div>Viatura: {group.viatura}</div>
                          <div>Período: {group.periodo}</div>
                        </div>
                      </TableCell>
                    ) : null}
                    <TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end space-x-1">
                        <Button variant="ghost" size="icon" title="Editar" onClick={() => onEditSchedule(agent.id)}>
                          <Edit className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" title="Substituir agente" onClick={() => onSubstituteAgent(agent.id)}>
                          <UserPlus className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" title="Remover turno" onClick={() => onDeleteShift(agent.id)}>
